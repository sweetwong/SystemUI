...
public class UsbDebuggingActivity extends AlertActivity
    ...
    public void onCreate(Bundle icicle) {
		...
		//[tanweian-20190423] add for the FactoryTest demand, Allows usb debugging and closes the dialog by default --- start
		if(isFactoryTestRunning("com.sen5.factorytest")){
			factoryTestAllowUsbDebugging();
		}
		//[tanweian-20190423] add for the FactoryTest demand, Allows usb debugging and closes the dialog by default --- end
	...
	//[tanweian-20190423] add for the FactoryTest demand, Allows usb debugging and closes the dialog by default --- start
	private void factoryTestAllowUsbDebugging(){
		try {
            IBinder b = ServiceManager.getService(USB_SERVICE);
            IUsbManager service = IUsbManager.Stub.asInterface(b);
            service.allowUsbDebugging(false, mKey);
        } catch (Exception e) {
            Log.e(TAG, "Unable to notify Usb service", e);
        }
        finish();
	...
	private boolean isFactoryTestRunning(String className)
    {
        boolean isTop = false;
        ActivityManager am = (ActivityManager)getSystemService(ACTIVITY_SERVICE);
        List<ActivityManager.RunningTaskInfo> runningTasks = am.getRunningTasks(8);
        for (ActivityManager.RunningTaskInfo runningTask : runningTasks){
            ComponentName topActivity = runningTask.topActivity;
            Log.d(TAG, "isTopActivity = " + topActivity.getClassName());
            if(topActivity.getClassName().contains(className)){
                isTop = true;
                break;
            }
        }
